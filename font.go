...
func LoadFont(fname string, size, flags int) *Font {
	...
	return (*Font)(C.al_load_font(cfname, C.int(size), C.int(flags)))
}
...
func (f *Font) GetTextWidth(text string) int {
	...
	return int(C.al_get_text_width((*C.ALLEGRO_FONT)(f), ctext))
}
...
func GrabFontFromBitmap(bmp *Bitmap, ranges []int) *Font {
	...
	return (*Font)(C.al_grab_font_from_bitmap((*C.ALLEGRO_BITMAP)(bmp),
...
func LoadBitmapFont(fname string) *Font {
	...
	return (*Font)(C.al_load_bitmap_font(cfname))
}

func CreateBuiltinFont() *Font {
	return (*Font)(C.al_create_builtin_font())
}